...
import { crawlFsItem } from "../../../src/dev/fs_crawl.ts";

export function clientSnapshot(options: ResolvedFreshViteConfig): Plugin {
  const modName = "fresh:client_snapshot";
        ...
        environments: {
          client: {
            build: {
              rollupOptions: {
                input: {
                  "client-snapshot": "fresh:client_snapshot",
                },
              },
            },
          },
...
if (import.meta.hot) {
  import.meta.hot.accept(() => {
    console.log("accepting client_snapshot")
  });
}
`;
    },
