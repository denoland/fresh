...
import { crawlFsItem, walkDir } from "../../../src/dev/fs_crawl.ts";
import { pathWithRoot, type ResolvedFreshViteConfig } from "../utils.ts";
import { specToName } from "../../../src/dev/builder.ts";
import { pathToSpec, UniqueNamer } from "../../../src/utils.ts";
import * as path from "@std/path";
import {
  generateSnapshotServer,
  type IslandModChunk,
  type PendingStaticFile,
} from "../../../src/dev/dev_build_cache.ts";
import { fsAdapter } from "../../../src/fs.ts";
import { getBuildId } from "./build_id.ts";

export function serverSnapshot(options: ResolvedFreshViteConfig): Plugin {
  const modName = "fresh:server_snapshot";
  ...
  return {
    ...
    async load(id) {
      ...
      let clientEntry = "/@id/fresh:client_entry";
      let buildId = "";
