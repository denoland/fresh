...
Deno.test({
  ...
  async fn(t) {
    ...
    await t.step("create conflicting route files", async () => {
      const encoder = new TextEncoder();
      const simpleTsData = `import { Handlers } from "$fresh/server.ts";
      ...
      `;
      let data = encoder.encode(simpleTsData);
      await Deno.writeFile(
        path.join(tmpDirName, "/routes/conflicting.ts"),
        data,
      );
      const simpleTsxData = `export default function Home(): null {
        throw new Error("boom!");
      }`;
      data = encoder.encode(simpleTsxData);
      await Deno.writeFile(
        path.join(tmpDirName, "/routes/conflicting.tsx"),
        data,
      );
    });

    await t.step("start dev mode and expect error", async () => {
      const cliProcess = new Deno.Command(Deno.execPath(), {
        args: [
          "run",
          "-A",
