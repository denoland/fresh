    ...
    const targetFileTree: FileTree[] = [
      {
        ...
        contents: [
          ...
          { type: "file", name: "fresh.gen.ts" },
          {
            type: "directory",
            name: "islands",
            contents: [{ type: "file", name: "Counter.tsx" }],
          },
          { type: "file", name: "main.ts" },
          {
            ...
            contents: [
              { type: "file", name: "[name].tsx" },
              {
                type: "directory",
                name: "api",
                contents: [{ type: "file", name: "joke.ts" }],
              },
          ...
          },
          {
            type: "directory",
            name: "static",
            contents: [{ type: "file", name: "logo.svg" }],
          },
        ],
      },
...
Deno.test({
  ...
  async fn(t) {
    ...
    const targetFileTree: FileTree[] = [
      {
        ...
        contents: [
          ...
          },
          {
            type: "directory",
            name: "utils",
            contents: [{ type: "file", name: "twind.ts" }],
          },
        ],
      },
    ...
    await t.step("start up the server and access the root page", async () => {
      ...
      const fontWeight = await counter?.evaluate(
...
Deno.test({
  ...
  async fn(t) {
    ...
    await t.step("start up the server with no options", async () => {
      const serverProcess = Deno.run({
        cmd: ["deno", "run", "-A", "--check", "main.ts"],
        stdout: "piped",
        stderr: "inherit",
      ...
      for await (const line of lines) {
        console.log(line);
        if (line.includes("Server listening on http://localhost:8000")) {
          started = true;
          break;
        }
      }
    ...
    await t.step("start up the server with port option", async () => {
      const serverProcess = Deno.run({
        cmd: ["deno", "run", "-A", "--check", "main.ts", "--port=8080"],
        stdout: "piped",
        stderr: "inherit",
        cwd: tmpDirName,
      });
      ...
      for await (const line of lines) {
        console.log(line);
        if (line.includes("Server listening on http://localhost:8080")) {
          started = true;
          break;
        }
      }
    ...
    await t.step("start up the server with hostname option", async () => {
      const serverProcess = Deno.run({
        cmd: [
          "deno",
          "run",
      ...
      for await (const line of lines) {
        console.log(line);
        if (line.includes("Server listening on http://127.0.0.1:8000")) {
          started = true;
          break;
        }
      }
    ...
    await t.step("print help instructions", async () => {
      const serverProcess = Deno.run({
        cmd: ["deno", "run", "-A", "--check", "main.ts", "--help"],
        stdout: "piped",
        stderr: "inherit",
        cwd: tmpDirName,
      });
