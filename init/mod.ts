...
const projectDir = path.resolve(unresolvedDirectory);
...
if (flags.tailwind == null) {
  if (confirm(`Set up ${colors.cyan("Tailwind CSS")} for styling?`)) {
    useTailwind = true;
  }
}
...
`;
  await writeProjectFile("Dockerfile", DOCKERFILE_TEXT);
}
...
`;
if (useTailwind) {
  await writeProjectFile("tailwind.config.ts", TAILWIND_CONFIG_TS);
}
...
await writeProjectFile("static/styles.css", cssStyles);
...
await writeProjectFile("static/logo.svg", STATIC_LOGO);

try {
  const res = await fetch("https://fresh.deno.dev/favicon.ico");
  const buf = await res.arrayBuffer();
  await writeProjectFile("static/favicon.ico", new Uint8Array(buf));
} catch {
...
`;
await writeProjectFile("README.md", README_MD);
...
if (useVSCode) {
  ...
  await writeProjectFile(".vscode/settings.json", vscodeSettings);

  const recommendations = ["denoland.vscode-deno"];
  if (useTailwind) recommendations.push("bradlc.vscode-tailwindcss");
  await writeProjectFile(".vscode/extensions.json", { recommendations });
  ...
  if (useTailwind) {
    ...
    await writeProjectFile(".vscode/tailwind.json", tailwindCustomData);
  }
}
