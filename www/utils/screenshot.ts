...
const pngPath = `./www/static/showcase/${id}.png`;
await page.screenshot({ path: pngPath });

await browser.close();

// convert to jpeg
const image2x = await Image.decode(await Deno.readFile(pngPath));
const jpeg2x = `./www/static/showcase/${id}2x.jpg`;
await Deno.writeFile(jpeg2x, await image2x.encodeJPEG(80));

const jpeg1x = `./www/static/showcase/${id}1x.jpg`;
const image1x = image2x.resize(image2x.width / 2, Image.RESIZE_AUTO);
await Deno.writeFile(jpeg1x, await image1x.encodeJPEG(80));

await Deno.removeSync(pngPath);
