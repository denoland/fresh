...
export async function collect(directory: string): Promise<Manifest> {
  const [routes, islands, actions] = await Promise.all([
    collectDir(join(directory, "./routes")),
    collectDir(join(directory, "./islands")),
    collectDir(join(directory, "./actions")),
  ]);

  return { routes, islands, actions };
}

export async function generate(directory: string, manifest: Manifest) {
  const { routes, islands, actions } = manifest;
...
${
    actions.map((file, i) => `import * as $$$${i} from "./actions${file}";`)
      .join("\n")
  }
...
const manifest = {
  ...
  actions: {
    ${
    actions.map((file, i) => `${JSON.stringify(`./actions${file}`)}: $$$${i},`)
      .join("\n    ")
  }
  },
...
export async function dev(base: string, entrypoint: string) {
  ...
  if (prevManifest) {
    ...
    currentManifest = { islands: [], routes: [], actions: [] };
  }
