import { FreshOptions } from "../server/types.ts";
import {
  dirname,
  extname,
  fromFileUrl,
...
interface Options {
  filenameIslands: true
}

export async function collect(directory: string, opts?: Options): Promise<Manifest> {
  const routesDir = join(directory, "./routes");
  const islandsDir = join(directory, opts?.filenameIslands ? "./components" : "./islands");
...
export async function generate(directory: string, manifest: Manifest, opts?: Options) {
...
${
    islands.map((file, i) => `import * as $$${i} from "${ opts?.filenameIslands ? "./components" : "./islands" }${file}";`)
      .join("\n")
  }
...
const manifest = {
  ...
  islands: {
    ${
    islands.map((file, i) => `${JSON.stringify(`${ opts?.filenameIslands ? "./components" : "./islands" }${file}`)}: $$${i},`)
      .join("\n    ")
  }
  },
...
export async function dev(base: string, entrypoint: string, opts?: Options) {
  ...
  const newManifest = await collect(dir, opts);
  ...
  if (manifestChanged) await generate(dir, newManifest, opts);
