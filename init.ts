...
export default function Home() {
  ...
  useTwind
    ...
    : ` style={styles.main}`
        ...
        <img
          src="/logo.svg"
          ${useTwind ? `class="w-32 h-32"` : `style={styles.image}`}
          alt="the fresh logo: a sliced lemon dripping with juice"
        />
        <p${
  useTwind ? ` class="my-6 text-center text-lg"` : ` style={styles.text}`
}>
          Welcome to{" "}
          <span${
...
const STYLES_INDEX_TS = `const styles = {
  main: {
    padding: "1rem",
    width: "100vw",
    height: "100vh",
    ...
    margin: "-1rem", // to prevent overflow
    fontFamily:
